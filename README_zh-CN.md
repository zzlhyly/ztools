...
- **站点爬取** - 爬取视频站点，解密 Fernet 保护的 API 响应，批量下载视频
...
站点爬取工具从 `sites.json`（不入 git 仓库）读取站点配置。复制模板并填入实际值：

```bash
cp sites.example.json sites.json
```

编辑 `sites.json`，配置你的站点信息：
...
工具支持任意使用相同架构的站点（Fernet 加密 CONFIG + REST API + 加密 `x-data` 响应）。可添加多个站点条目。
