...
describe('HASH_ALGORITHMS', () => {
  it('contains 7 algorithms', () => {
    expect(HASH_ALGORITHMS.map((a) => a.value)).toEqual([
      'MD5',
      'SHA-1',
      'SHA-256',
      'SHA-384',
...
describe('calculateMd5', () => {
  ...
  it('"The quick brown fox jumps over the lazy dog" -> 9e107d9d372bb6826bd81d3542a419d6', () => {
    expect(calculateMd5('The quick brown fox jumps over the lazy dog')).toBe(
...
describe('calculateHash', () => {
  ...
  it('SHA-256', async () => {
    expect(await calculateHash('hello', 'SHA-256')).toBe(
