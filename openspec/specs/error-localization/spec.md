...
Tauri command failures are categorized with machine-readable error codes (`FILE_NOT_FOUND`, `PERMISSION_DENIED`, `NETWORK_ERROR`, `UNKNOWN`), wrapped as `TauriError` instances, and surfaced with localized messages matching the current application locale. Raw Rust error strings are never shown to the user.
